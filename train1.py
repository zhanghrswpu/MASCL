...
from torchvision.datasets import MNIST  # 新增：MNIST 数据集

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# 加载扩散模型的预训练权重 根据标签生成图像
save_dir = "./mnist"  # 改为 MNIST 权重路径
...
class DDIMSampler(torch.nn.Module):
    ...
    def forward(self, x_T, labels, sample_steps=500, eta=0.):
        device = x_T.device
...
def train(args):
    ...
    def generated_image_method_batch(labels, sample_steps=500, eta=0.0):

        B = labels.shape[0]
        xT = torch.randn((B, 1, 32, 32), device=DEVICE)
        with torch.no_grad():
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if epoch % 10 == 0:
            save_path = os.path.join(config.save_path, f'mnist_model_stage1_apart1_epoch{epoch}.pth')
