...
def eval(modelConfig: Dict):
    ...
    with torch.no_grad():
        ...
        noisyImage = torch.randn(size=[1, 1, modelConfig["img_size"], modelConfig["img_size"]], device=device)
...
def eval(modelConfig:Dict):
    device = torch.device(modelConfig["device"])
    target_label = modelConfig["label"]
    ...
    with torch.no_grad():
        ...
        noisyImage = torch.randn(
            size=[1, 3, modelConfig["img_size"], modelConfig["img_size"]], device=device)  # 批次大小为1
        ...
        # 生成图像
        sampledImgs = sampler(noisyImage, labels)
        sampledImgs = sampledImgs * 0.5 + 0.5  # 归一化到 [0, 1]
        
        # 保存生成的图像
        save_path = os.path.join(modelConfig["sampled_dir"], f"sampled_label_{target_label}.png")
        save_image(sampledImgs, save_path)
